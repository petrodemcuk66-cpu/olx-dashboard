...
Підтримує CSV з українськими і англійськими назвами колонок.
"""
...
# Мапи українських назв колонок → англійських
COL_MAP = {
    "Назва": "title",
    "Категорія": "category",
    "Місто": "city",
    "Ціна (грн)": "price_uah",
    "Ціна (сирова)": "price_raw",
    "Торг": "negotiable",
    "Бізнес": "is_business",
    "Дата": "date_parsed",
}
...
def load_data() -> pd.DataFrame:
    if DATA_PATH.exists():
        ...
        # Якщо колонки українські — перейменовуємо
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Числові
    df["price_uah"] = pd.to_numeric(df.get("price_uah"), errors="coerce")
...
def listings_by_date(df: pd.DataFrame, days: int = 30) -> pd.DataFrame:
    ...
    result = (
        recent.groupby("date_parsed").size().rename("count")
...
def heatmap_city_category(df: pd.DataFrame, top_cities: int = 8) -> pd.DataFrame:
    ...
    pivot = filtered.pivot_table(
        values="price_uah", index="city", columns="category_ua", aggfunc="median"
    ).round(0)
    return pivot.reindex(top)
...
def freshness(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        df2.groupby("age_group", observed=True).size()
